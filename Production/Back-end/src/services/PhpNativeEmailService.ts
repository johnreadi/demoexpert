/**
 * Service d'email utilisant la fonction mail() PHP native
 * Compatible o2Switch - Version de production
 */
...
class PhpNativeEmailService {
  ...
  async sendEmail(options: EmailOptions): Promise<EmailResult> {
    try {
      ...
      const result = await this.sendPhpNativeEmail(toHeader, subject, content, headers);

      if (result.success) {
        console.log(`✅ Email envoyé via PHP native à ${toHeader}`);
        return {
          success: true,
  ...
  private buildHeaders(options: { from: string; replyTo: string; subject: string }): string {
    ...
    const headers = [
      ...
      `X-Mailer: DemolitionExpert-PHP/1.0`,
      `X-Priority: 3`,
      `Return-Path: ${from}`,
      `Subject: ${subject}`
    ];
   ...
   */
  private async sendPhpNativeEmail(to: string, subject: string, content: string, headers: string): Promise<{success: boolean; messageId?: string; error?: string}> {
    return new Promise((resolve) => {
      // Simulation de l'envoi d'email en mode développement
      if (!this.isProduction) {
        console.log('📧 [MODE DEV] Email simulé:');
        console.log(`📨 À: ${to}`);
        console.log(`📝 Sujet: ${subject}`);
        console.log(`📄 Contenu: ${content.substring(0, 200)}...`);

        const messageId = `dev-${Date.now()}-${Math.random().toString(36).substr(2, 9)}@demolition-expert.com`;
      ...
      // Envoi réel en production via PHP native
      try {
        const { exec } = require('child_process');
        const phpScript = this.generatePhpScript(to, subject, content, headers);

        exec(`php -r "${phpScript}"`, (error: any, stdout: string, stderr: string) => {
          if (error) {
          ...
          if (stderr) {
            console.warn('⚠️ Avertissement PHP:', stderr);
          }

          const messageId = `php-${Date.now()}-${Math.random().toString(36).substr(2, 9)}@demolition-expert.com`;

          console.log('✅ Email envoyé via PHP native');
          console.log('📊 Sortie PHP:', stdout);
        ...
        console.error('❌ Erreur lors de l\'envoi PHP:', error);
        resolve({
          success: false,
          error: 'Service d\'email non disponible'
        });
      }
    });
  }
  ...
  /**
   * Génération du script PHP pour l'envoi d'email
   */
  private generatePhpScript(to: string, subject: string, content: string, headers: string): string {
    // Échappement des caractères spéciaux pour PHP
...
if ($result) {
    ...
    echo "Message-ID: " . uniqid() . "@demolition-expert.com\\n";
} else {
...
export default PhpNativeEmailService;
