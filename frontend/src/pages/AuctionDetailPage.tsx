...
export default function AuctionDetailPage(): React.ReactNode {
        ...
        const fetchAuction = async () => {
            ...
            try {
                ...
                if(foundAuction) {
                    ...
                    setPageError("Cette offre n'a pas été trouvée.");
                }
            } catch (err) {
                console.error("Failed to fetch auction", err);
                setPageError("Impossible de charger les détails de l'offre.");
            } finally {
                setIsLoading(false);
            }
        };
    ...
    const handleBidSubmit = async (e: React.FormEvent) => {
        ...
        if (isNaN(newBid) || newBid <= auction.currentBid) {
            setBidError(`Votre offre doit être supérieure à ${auction.currentBid.toLocaleString()} €.`);
            setSuccess('');
            return;
        }
        ...
        try {
            ...
            setBidError(err.message || "Une erreur est survenue lors de l'envoi de l'offre.");
        } finally {
            setIsBidding(false);
        }
    };
                ...
                <div className="mt-12">
                     ...
                     <div className="overflow-x-auto">
                        <table className="w-full text-left">
                            ...
                            <tbody>
                                {auction.bids.slice().sort((a, b) => new Date(b.timestamp).getTime() - new Date(a.timestamp).getTime()).map((bid, index) => (
                                    <tr key={index} className="border-b">
                                        ...
                                        <td className="p-3 text-sm text-gray-600">{new Date(bid.timestamp).toLocaleString('fr-FR')}</td>
                                    </tr>
                                ))}
                            </tbody>
                        </table>
